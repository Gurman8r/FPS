...
namespace FPS
{
    ...
    public class PlayerController : BaseUnitController
    {
        ...
        * * * * * * * * * * * * * * * */
        ...
        [SerializeField] BaseItem   m_editing;
        ...
        * * * * * * * * * * * * * * * */
        ...
        private bool CheckInteraction(RaycastHit hit)
        {
            ...
            {
                BaseItem item;
                if ((item = hit.transform.GetComponent<BaseItem>()) && item.interactable && !item.owner)
                {
                    hud.ShowActions(string.Format(
                        "[{0}] {1} {2}", 
        ...
        private void UpdateHand(UnitInventory.Hand hand)
        {
            ...
            if (item = hand.item)
            {
                ...
                m_zoomLevel = (gun = item as BaseRangedWeapon) ? gun.zoomLevel : FirstPersonCamera.MinZoom;
        ...
        private void SetEditItem(BaseItem value)
        {
            if(value)
            {
                m_editing = value;
